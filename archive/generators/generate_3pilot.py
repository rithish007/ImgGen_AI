"""Stage 1 generation - 3-pilot: full-precision, no-quantization comparison.

Duplicated from generate.py rather than editing it in place, so the already-
verified 2-pilot klein/flux2dev(fp8) code path stays untouched and
reproducible. This file's only real difference: flux2dev drops fp8
quantization entirely and instead splits its two large components across 2
GPUs, so every model in this round runs at full bf16 precision with zero
quantization anywhere - the explicit point of the 3-pilot (see chat: klein
was already unquantized, this makes flux2dev match, and generate_hunyuan.py
runs Hunyuan unquantized too via device_map="auto" across 4 GPUs).
...
_load_flux2dev_multi_gpu() below before debugging from scratch.
    ...
    python -m archive.generators.generate_3pilot --model flux2dev --manifest manifests/2-pilot.json --limit 3 --out outputs/3-pilot/flux2dev_smoke

    # full run
    python -m archive.generators.generate_3pilot --model flux2dev --manifest manifests/2-pilot.json --out outputs/3-pilot/flux2dev
...
def main() -> None:
    ...
    out_dir = args.out or Path("outputs") / "3-pilot" / args.model
    if not args.dry_run:
