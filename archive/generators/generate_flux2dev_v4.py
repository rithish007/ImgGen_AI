"""Stage 1 generation - 5-pilot v4: flux2dev with the starfish multi-class
undercount fix and the expanded scene/algae/lighting pools.

Duplicated from generate_3pilot_v3.py rather than editing it in place. Only
real differences: imports build_prompt() from prompts_v4.py (starfish
recoloured + camouflage language dropped from arrangement text, addressing
a measured 0.54x mean detection ratio on multi-class starfish rows vs 0.91x
single-class; SCENE_TEMPLATES/ALGAE_VARIATIONS/LIGHTING_CONDITIONS expanded
with genuinely different content instead of near-synonymous paraphrases -
see prompts_v4.py's module docstring for the full evidence behind both) and
defaults output to outputs/5-pilot/flux2dev_v4 instead of
outputs/5-pilot/flux2dev_v3, to sit alongside the v2/v3 runs for direct
comparison. Token budget re-verified against the real tokenizer after both
changes: worst case across all 50 manifest rows is 435/512 (v3's was 479) -
...
_load_flux2dev_multi_gpu() below before debugging from scratch.
    ...
    python -m archive.generators.generate_flux2dev_v4 --model flux2dev --manifest manifests/2-pilot.json --limit 3 --out outputs/flux2dev/v4/smoke

    # full run
    python -m archive.generators.generate_flux2dev_v4 --model flux2dev --manifest manifests/2-pilot.json --out outputs/flux2dev/v4
...
def main() -> None:
    ...
    out_dir = args.out or Path("outputs") / "flux2dev" / "v4"
    if not args.dry_run:
            ...
            json.dumps(
                {
                    ...
                    "model_repo": cfg["repo"],
                    "prompt_variant": "v4_starfish_multiclass_fix",
                    "prompt": prompt,
                    "seed": row["seed"],
                    "steps": steps,
                    "guidance": guidance,
