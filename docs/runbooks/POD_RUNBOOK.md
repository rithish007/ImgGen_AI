...
```bash
cd /workspace/ImgGen_AI && git pull && python tools/pod_preflight.py
```
...
```bash
cd /workspace/ImgGen_AI && python -m archive.generators.generate --model klein --manifest manifests/smoke.json
```

Full pilot (20 images per model, ~22 min each) only once the smoke test looks right:

```bash
cd /workspace/ImgGen_AI && python -m archive.generators.generate --model klein --manifest manifests/pilot.json
```

```bash
cd /workspace/ImgGen_AI && python -m archive.generators.generate --model sd35 --manifest manifests/pilot.json
```
