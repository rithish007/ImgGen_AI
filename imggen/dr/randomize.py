...
THREE PROFILES, selected by --profile:
    ...
    duo_calibrated - dataset D. Diagnosed 2026-08-19/20: {3C,5C,7C} of the
        anchored profile's own beta/c(lambda) table have RED attenuating
        SLOWER than green (c_red/c_green < 1 - confirmed directly from
        jerlov_anchored._C_BLUE_GREEN_RED), the opposite of DUO's measured
        real-photo colour cast (ratio_rg=0.438, strongly green-dominant - see
        reports/water_stats/duo_test_water_stats.json). Only 1C has the correct sign
        (1.19). Since "anchored" samples water_type uniformly across all
        four, its output averages toward the wrong direction more often than
        not - empirically confirmed: an anchored-profile pilot batch trained
        a WORSE real-world detector than the simpler placeholder profile.
        duo_calibrated fixes this two ways: (1) direct-signal beta is pinned
        to 1C (the one type with correct sign for BOTH beta and kd - see
        verification thread), keeping physically-real magnitude via the same
        beta_b_anchor_range() as the anchored profile; (2) the veiling light
        B_inf is centred directly on DUO's own measured airlight estimate
        (DUO_VEILING_LIGHT_RGB below - computed by imggen/analysis/water_stats.py on the
        exact 778-image dataset/real_eval test split this project evaluates
        against) instead of derived indirectly via a per-type Kd table and
        sampled depth d. Validated empirically (50-image test batch, resized
        to DUO's native 640px before comparing - dark_channel_mean is NOT
        resolution-invariant, confirmed by direct test, so this matters):
        closes ~60% of the anchored profile's ratio_rg gap and ~70% of its
        ratio_bg gap vs DUO's real measured statistics. Known remaining
        weakness: overshoots DUO's dark_channel_mean (haze) by roughly the
        same absolute margin anchored undershoots it - three attempted fixes
        for this (wider beta_b range, lower visibility_floor, halved blend
        strength) all made the AGGREGATE match worse, since they trade away
        more color-cast accuracy than they recover in haze accuracy (the
        color-cast gap is 5-8x larger in magnitude than the haze gap). Kept
        as-is on the judgment that color-cast is very plausibly the more
        detection-relevant statistic; this is a pixel-statistics proxy
        judgment, not something verified against actual detector accuracy
        until the training run this profile exists to feed.
...
reproducibility):
    ...
    - vertical depth d ~ U(0, 5) m - sets the veiling light B_c^inf via
      imggen/dr/jerlov.py's kd_rgb() (see that file for the flagged simplification
...
see AI_Pipeline_Test_Plan.md.
    ...
    python -m imggen.dr.randomize --images-dir outputs/1-pilot/klein --range-dir outputs/1-pilot/range
...
PROFILE_WATER_TYPES = {
    ...
    "duo_calibrated": ["1C"],  # pinned - see module docstring's duo_calibrated section
}

DUO_CALIBRATED_WATER_TYPE = "1C"
# He/Sun/Tang airlight estimate, aggregate mean over dataset/real_eval/images
# (the 778-image DUO test split), from reports/water_stats/duo_test_water_stats.json,
# produced by: python -m imggen.analysis.water_stats --images-dir dataset/real_eval/images
DUO_VEILING_LIGHT_RGB = (0.417, 0.800, 0.535)
DUO_TARGET_JITTER_FRAC = 0.15  # per-image multiplicative jitter around the DUO
# veiling-light centre, so duo_calibrated still injects DR diversity rather
# than collapsing every image toward one fixed backscatter colour
...
def sample_params(seed: int, z_far: float, profile: str = "placeholder") -> dict:
    ...
    params = {
        ...
        "d": rng.uniform(*d_range),  # drawn for RNG-stream consistency across profiles;
        # NOT used to derive B_inf under duo_calibrated (see transform_image) - kept so
        # the config JSON's draw sequence lines up with the other two profiles' and so
        # switching a run between profiles doesn't reshuffle downstream draws (noise/
        # blur/vignette) for the same seed.
    ...
    if profile == "duo_calibrated":
        # appended after all other draws (not interleaved) so placeholder/anchored's
        # draw sequences are completely untouched by this profile's existence
...
def transform_image(j_img, z, params: dict):
    ...
    if params["profile"] == "placeholder":
        ...
        # anchored and duo_calibrated share the same 1C-anchored direct-signal beta
        # (duo_calibrated's water_type is pinned to "1C" via PROFILE_WATER_TYPES)
    ...
    if params["profile"] == "duo_calibrated":
        # B_inf centred directly on DUO's own measured veiling-light colour instead
        # of derived via kd/d - see module docstring's duo_calibrated section for why
        # (jerlov_anchored's per-type Kd table isn't the fix's active ingredient; kd_r
        # is unused here deliberately, not an oversight)
