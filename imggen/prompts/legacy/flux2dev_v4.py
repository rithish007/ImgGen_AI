...
prompts_v4.py
=============
Prompt engine v4 - two more fixes on top of v3, both for flux2dev: the
starfish multi-class undercount, and a genuine (not paraphrase) expansion of
the scene/algae/lighting pools to address the "scenes repeat" finding.

Duplicated from prompts_v3.py rather than editing it in place (same
...
Self-test (token budget check against the real tokenizer):
    ...
    from imggen.prompts.legacy.flux2dev_v4 import build_prompt
    import json
...
CLASSES: dict[int, dict[str, object]] = {
    ...
    0: {
        ...
        # module docstring for that evidence). Reverted to the original
        # brown-grey/muted palette per direct feedback after the smoke test:
        # the orange read as too vibrant/artificial. NOTE: this brings back
        # the low-contrast-against-substrate condition the recolour was
        # meant to fix - the multi-class undercount evidence in the module
        # docstring still applies, it just isn't being addressed by colour
        # anymore. Watch multi-class starfish counts in the next full run;
        # if the undercount reappears, the arrangement-text fix below (still
        # in place) may not be enough on its own.
        "morphology": [
...
# v4: expanded 5 -> 8 with genuinely different environments, not paraphrases
# of the same one (see module docstring - the original 5 were all reworded
# "sand, gravel, rocks" and rendered visually near-identical across indices).
SCENE_TEMPLATES = [
    ...
    "kelp holdfast zone - thick brown kelp stipes rising from a rocky seabed, fronds partly overhead",
    "silty low-lying flat - fine dark sediment, sparse small rocks, occasional shell fragments",
...
# v4: expanded 5 -> 6 with genuinely different growth types/density, not
# reworded "sparse algae on rocks".
ALGAE_VARIATIONS = [
    ...
    "small patches of pink coralline algae encrusting rock",
    "scattered strands of loose kelp debris resting on the seabed",
...
# v4: expanded 4 -> 7 with genuinely different lighting moods, not reworded
# "soft natural daylight" (see module docstring). Kept "clear water" implied
# throughout - Stage 3 owns turbidity/water-clarity, this pool only varies
# light quality/angle, not visibility, so nothing here contradicts
# SCENE_WATER_PHRASE.
LIGHTING_CONDITIONS = [
    ...
    "golden low-angle light, long soft shadows",
]
