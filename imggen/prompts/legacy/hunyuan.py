...
prompts_hunyuan.py
===================
Hunyuan-specific fork of prompts.py - duplicated rather than edited in place
so the shared klein/flux2dev prompt engine stays untouched and reproducible.

Two SUBTRACTIVE changes only, not another additive patch. 3-pilot visual
audit found a real, locatable contradiction: a "dense" scene simultaneously
told the model "overlapping environmental features" (SCENE_DENSITIES) AND
"individuals touching or partially overlapping" (per-class arrangement) AND,
in the single-class/high-count/rock-formation combination specifically,
carried an implicit push toward clustering everything around the one salient
rock feature. Result: sea urchins fusing into one anatomically confused mass
in dense clusters (bald-dome + fused-cluster defects, 3-pilot Hunyuan run),
and the same combination separately producing decoratively symmetric
compositions in the 4-pilot flux2dev experiment. Two independent symptoms of
the model being asked to reconcile contradictory density signals.

Fix philosophy per user direction: consolidate to ONE spatial-relationship
signal instead of three, and gate structurally (skip a clause in code) for
the identified high-risk row shape instead of writing another sentence to
constrain it. Do not keep adding corrective clauses - that's what produced
the contradiction in the first place.

1. SCENE_DENSITIES["dense"] reworded to drop "overlapping environmental
   features" - that phrase is ambiguous between "terrain features overlap"
   (fine) and "target objects overlap" (redundant with, and here
   contradicting, the per-class arrangement text, which is the correct sole
   owner of target-object spatial relationships).
2. Rock-formation clause is now skipped entirely (not reworded, not
   constrained by an added sentence) when a row is single-class AND
   density="dense" - the specific combination behind both the 4-pilot
   clustering issue and the 3-pilot urchin fusion/bald-dome defects. rock_
   formation_idx is still drawn from the rng each call (keeps the random
   sequence stable / directly comparable to prompts.py for shared seeds)
   even when its text is omitted.

No negative_prompt split: checked Hunyuan's actual generate_image() signature
(captured live via inspect.signature during the 3-pilot smoke test) plus
Tencent's own run_image_gen.py usage script - neither exposes a negative/cfg
parameter. The underlying diffusion decoder does have real classifier-free
guidance internally (ClassifierFreeGuidance, guidance_scale, conditional/
unconditional prediction blending, confirmed by reading
hunyuan_image_3_pipeline.py) but nothing in either official source suggests
the unconditional branch is user-customizable - almost certainly a fixed
empty-string default, standard CFG practice. So exclusions still fold into
the positive prompt via POSITIVE_ONLY_GUARDS, same as before.

======================================================================
Original prompts.py docstring follows, unchanged below this point.
======================================================================
...
SCENE_DENSITIES = {
    ...
    # "overlapping environmental features" (previous wording) was ambiguous
    # between terrain overlap (fine) and target-object overlap (redundant
    # with, and contradicting, the per-class arrangement text - see this
    # file's module docstring). Reworded to describe clutter without implying
    # anything about how close target objects are to each other; the
    # per-class arrangement is now the sole owner of that concept.
    "dense": (
        "visually cluttered natural seabed containing rocks, algae, gravel, "
        "sediment and closely spaced natural debris, while maintaining "
...
# reports/class_counts/class_counts.json showed scallop instances in far more images (17/20)
...
def build_prompt(
    ...
    *,
    ...
    # Still drawn unconditionally to keep the rng sequence stable and
    # directly comparable to prompts.py for a shared seed - only whether its
    # text gets INCLUDED below is conditional, not whether it's drawn.
    rock_formation, rock_formation_idx = _choose(rng, ROCK_FORMATIONS)

    # Single-class + dense is the specific row shape behind both the 4-pilot
    # decorative-clustering issue and the 3-pilot urchin fusion/bald-dome
    # defects - see module docstring. Gated structurally (omit the clause)
    # rather than adding another sentence to constrain it.
    ...
    prompt_parts = [
        ...
        (
            ...
            + (
                ...
                if include_rock_formation else ""
            )
        ),
