...
rock, shell tones) - not vivid/saturated, but not chasing a specific water
cast either, since that's not this stage's job.

WHAT CHANGED: COLOR_PALETTE_GUARD reworded from a whole-image instruction
("muted colour palette... not vivid or saturated") to one scoped explicitly
to materials ("seabed and shell material in muted natural tones..."),
removing the implicit whole-frame colour-grading instruction that was
steering the water-adjacent pixels too. CLASSES morphology text (which
already specifies muted colours per-species - brown-grey starfish, blue-grey
variant, etc.) is unchanged; this guard was always meant to reinforce that,
not add a second, broader colour instruction on top of it.

--- prompts_v6.py's own docstring follows for everything this file didn't
touch (camera-distance/subject-scale experiment, all still accurate) ---

Prompt engine v6 - EXPERIMENTAL. A much bigger camera-distance escalation
than anything tried before, on top of v5.

Two prior attempts at fixing the "product photography" look both came back
negative: swapping the FRAMING label (close-up->mid) barely changed the
rendered image at all, and forcing camera_height="high" (CAMERA_HEIGHTS'
existing top tier, "~1.5-2m up") did nothing either - see prompts_v5.py's
docstring and the flux2dev_v5_highcam_smoke test. Both attempts stayed
within a fairly narrow band (0.5m-2m). User's next request: test a much
bigger jump - camera 5-8m above the seabed, not 1.5-2m - on the theory that
the earlier tests weren't a big enough distance change to actually move the
model. Combined with a second, previously-untested lever: explicit
subject-scale language ("small in frame"), since camera-position language
alone has failed twice - pairing it with a direct instruction about how
large the subject should read in the frame is a different mechanism, not
just a bigger version of the same failed one.

NOT touching SCENE_WATER_PHRASE / water colour/clarity. Stage 3's
Akkaynak-Treibitz domain-randomization transform (imggen/dr/randomize.py)
already owns depth-based light attenuation and colour cast as a separate,
controllable post-process - Stage 1 (this file) deliberately renders clear,
colour-neutral water on purpose, see SCENE_WATER_PHRASE's own comment. Doing
depth-based colour grading in BOTH stages would double-degrade the same
thing Stage 3 already handles. So "5-8m below sea level" is read here as a
camera-DISTANCE instruction (camera 5-8m above the subjects, wide overview
survey framing), not a water-colour/turbidity instruction - if that's not
what was meant, this needs revisiting before trusting the result.

WHAT CHANGED: added CAMERA_HEIGHTS["far"] = "camera ~5-8m up, wide overview
survey view" (existing top tier "high" was only ~1.5-2m - this is a genuinely
different order of magnitude, not an incremental bump). Added
SUBJECT_SCALE_GUARD, a new short guard reinforcing that organisms should
read as small within a wide seabed view, not filling/dominating the frame -
untested lever, paired here because camera-position text alone has failed
twice already. Not added to the CAMERA_HEIGHTS random pool - this version's
generate script (generate_3pilot_v6.py) FORCES camera_height="far" on every
row for a clean, isolated test, same pattern as the earlier highcam
experiment. Token budget re-verified after both additions - see below.

--- prompts_v5.py's own docstring follows for everything this file didn't
touch (colour palette, no-kelp scene pools, starfish blue variant, count
fixes, etc. - all still accurate) ---
...
Self-test (token budget check against the real tokenizer):
    ...
    from imggen.prompts.legacy.flux2dev_v7 import build_prompt
    import json
...
# v6: added "far" - a genuinely different order of magnitude from "high"
# (1.5-2m), not an incremental bump. Not in the random pool - forced
# explicitly by generate_3pilot_v6.py for a clean, isolated test.
CAMERA_HEIGHTS = {
...
# v7: narrowed to materials only - see module docstring for the water_stats.py
# evidence. No longer a whole-image colour-grading instruction (that was
# measurably pulling the aggregate pixel colour away from DUO's real
# green-dominant water, not toward it - Stage 3 owns water colour, not this).
COLOR_PALETTE_GUARD = "seabed, rock and shell material in muted natural tones, not vivid or saturated"

# v6: new, untested lever - camera-position text alone (framing label,
# camera_height) has failed twice to change the "product photography" look,
# so this targets the mechanism directly instead: how large the subject
# reads in the frame. Paired with camera_height="far", not a replacement
# for it.
