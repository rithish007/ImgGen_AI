...
assemble_v9_combined_dataset.py) with their duo_calibrated_scatter-profile
DR'd copies (imggen/dr/randomize_v2.py - color calibration plus the
scattering/contrast/noise stage).

Four images per image_id group: v9 base, v9 base+duo_calibrated_scatter DR,
v9_starfish, v9_starfish+duo_calibrated_scatter DR - up to 4000 images total.
...
Run after both sets have duo_calibrated_scatter DR'd copies + SAM3 labels:
    outputs/flux2dev/v9/dr_runs/v1/dr_duo_calibrated_scatter/          (v9 base)
    outputs/flux2dev/v9_starfish/dr_runs/v1/dr_duo_calibrated_scatter/ (v9_starfish)
    outputs/flux2dev/v9/labels/sam3/                                  (v9 base labels)
    outputs/flux2dev/v9_starfish/labels/sam3/                         (v9_starfish labels)

    python -m imggen.data.assemble_v9_combined_duo_scatter_dataset
"""
...
def main() -> None:
    ...
    ap.add_argument("--v9-dr-images-dir", type=Path, default=Path("outputs/flux2dev/v9/dr_runs/v1/dr_duo_calibrated_scatter"))
    ...
    ap.add_argument("--starfish-dr-images-dir", type=Path, default=Path("outputs/flux2dev/v9_starfish/dr_runs/v1/dr_duo_calibrated_scatter"))
    ap.add_argument("--out-dir", type=Path, default=Path("dataset/v9_flux2dev_combined_duo_scatter_dr"))
    ...
    print(f"{len(stems)} image_id groups, {missing_v9_dr} missing v9 duo_calibrated_scatter DR, {missing_sf_dr} missing starfish duo_calibrated_scatter DR")
