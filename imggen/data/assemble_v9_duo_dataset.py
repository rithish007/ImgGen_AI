...
their duo_calibrated DR'd copies (base images get domain-randomized into
...
An earlier version of this script pointed at a separate
outputs/flux2dev/v9/dr_runs/v1/labels/sam3_duo_calibrated/ directory built by
re-running SAM3 on the recolored images; comparing the two against the base
labels (IoU>=0.5 per class, all 1000 images) showed SAM3 recovered only 87.6%
of the base instances on the duo_calibrated pixels (12.4% silently missing,
worst cases losing most of a scene's boxes) - a real detection-recall gap
...
diversity. Reusing the base label file removes that gap entirely.
    ...
    python -m imggen.data.assemble_v9_duo_dataset
"""
...
def main() -> None:
    ...
    ap.add_argument("--dr-images-dir", type=Path, default=Path("outputs/flux2dev/v9/dr_runs/v1/dr_duo_calibrated"))
    ap.add_argument("--dr-suffix", default="_duo_dr")
    ap.add_argument("--out-dir", type=Path, default=Path("dataset/v9_flux2dev_duo_dr"))
