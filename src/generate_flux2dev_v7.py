...
_load_flux2dev_multi_gpu() below before debugging from scratch.
    ...
    python src/generate_flux2dev_v7.py --model flux2dev --manifest manifests/2-pilot.json --limit 3 --out outputs/flux2dev/v7/smoke

    # full run
    python src/generate_flux2dev_v7.py --model flux2dev --manifest manifests/2-pilot.json --out outputs/flux2dev/v7
...
def main() -> None:
    ...
    out_dir = args.out or Path("outputs") / "flux2dev" / "v7"
    if not args.dry_run:
            ...
            json.dumps(
                {
                    ...
                    "model_repo": cfg["repo"],
                    "prompt_variant": "v7_material_color_guard",
                    "prompt": prompt,
                    "seed": row["seed"],
                    "steps": steps,
                    "guidance": guidance,
