...
only the prompt text differs) to isolate the prompt's effect.
    ...
    python src/generate_3pilot_promptfix.py --model flux2dev --manifest manifests/2-pilot.json --limit 3 --out outputs/3-pilot/flux2dev_promptfix_smoke

    # full run
    python src/generate_3pilot_promptfix.py --model flux2dev --manifest manifests/2-pilot.json --out outputs/3-pilot/flux2dev_promptfix
...
def main() -> None:
    ...
    out_dir = args.out or Path("outputs") / "3-pilot" / f"{args.model}_promptfix"
    if not args.dry_run:
        out_dir.mkdir(parents=True, exist_ok=True)

    print(f"model={args.model}  stage=3-pilot-promptfix  rows={len(rows)}")
