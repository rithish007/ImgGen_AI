"""
prompts_hunyuan_v3.py
======================
EXPERIMENTAL. Same as prompts_hunyuan_v2.py (camera/POV fix, ground-contact
fix, DUO colour-palette/no-kelp fixes - all unchanged, see that file's own
docstring) plus a much bigger camera-distance escalation: CAMERA_HEIGHTS
now has a "far" tier (~5-8m above the seabed, vs "high"'s 1.5-2m) and a new
SUBJECT_SCALE_GUARD ("small in frame" language). Same rationale as
prompts_v6.py (flux2dev's equivalent test) - two prior attempts at fixing
the "product photography" look, both in the narrow 0.5-2m band, both came
back negative. Not touching water colour/clarity - Stage 3 owns that
separately, see prompts_v6.py's docstring for the full argument.

Duplicated from prompts_hunyuan_v2.py rather than editing it in place -
that file has already produced real output (smoke_hunyuan_povfix,
test_hunyuan_instruct), same "don't edit files that produced real output"
...
# v3 (this file): new, untested lever - camera-position text alone has
# failed twice on flux2dev to change the "product photography" look, so
# this targets the mechanism directly: how large the subject reads in the
# frame. Paired with camera_height="far", not a replacement for it.
SUBJECT_SCALE_GUARD = "every organism reads as small within the wider seabed view, not filling or dominating the frame - a distant survey vantage, not a close specimen shot"
