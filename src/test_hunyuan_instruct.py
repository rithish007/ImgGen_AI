...
generation pipeline. Instruct is a different checkpoint from the Pretrain one
this project actually uses (tencent/HunyuanImage-3.0, see generate_hunyuan.py/
...
RAW_INSTRUCTION = (
    ...
    "just above the seafloor, as if the viewer is looking through the "
    "camera's own lens. Show exactly 2 starfish, 2 sea urchins and 3 "
    "scallops, each resting naturally and in direct physical contact with "
    "the substrate, on a rocky, moderately algae-covered temperate coastal "
    "seabed with sand and gravel. Documentary survey style, candid and "
    "unposed, not a specimen display - no divers, boats, CG rendering, or "
    "any part of the survey vehicle (arms, thrusters, camera housing) "
    "visible in frame. Natural diffuse daylight, true-to-life colour, full "
    "frame with no vignette."
)


def _build_structured_prompt() -> str:
    """The same request, built through prompts_hunyuan_v2.py's normal
    pipeline - i.e. what this project's hand-engineered path would actually
    send Hunyuan for this row, for a direct side-by-side against
...
def main() -> None:
    ...
    ap.add_argument("--out", type=Path, default=Path("outputs") / "hunyuan" / "instruct")
    args = ap.parse_args()
    ...
    if args.which in ("raw", "both"):
        _generate(model, RAW_INSTRUCTION, "raw_instruction", args.out)
    if args.which in ("structured", "both"):
        _generate(model, _build_structured_prompt(), "structured_prompt", args.out)
